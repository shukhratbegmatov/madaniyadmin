  ...
  <div>
    ...
    <div class=" main_card">
      <div class="mains_card_child">
        <div class="card">
          <div class="p-3">
            <router-link to="/council-advice/create"
          ...
          <table class="table table-flush" id="datatable">
            ...
            <tbody v-if=" $store.state.council_advice">

            <tr v-for="(item,index) in  $store.state.council_advice" :key="index">

              <td><div class="father_tabel">{{ item.id }}</div></td>
              <td><div class="father_tabel">{{ item.task }}</div></td>
              <td><div class="father_tabel">{{ item.created_on }}</div></td>
              <td><div class="father_tabel">{{ item.updated_on }}</div></td>
              <td>
                <router-link :to="'/council-advice/edit?id='+item.id" class="btn">
...
export default {
  ...
  mounted() {
    this.$store.dispatch('councilAdvice',{
  ...
  methods: {
    ...
    deletes(item) {
      ...
      if (isBoss == true) {
        this.$http.delete('/api/council-advice/' + item + '/',
            {
              headers: {
