      ...
      <div class="row">
        ...
        <div class="col">
          <div class="buttons_create">
            <div>
              <h3 class="font-weight-bold mb-3">Медиатека</h3>
            </div>
      ...
      <div class="mains_card_child">
        <div class="card">
          ...
          <table class="table table-flush" id="datatable">
            ...
            <tbody v-if=" $store.state.files">

            <tr v-for="(item,index) in  $store.state.files.results" :key="index">

              <td><div class="father_tabel">{{ item.file_id }}</div></td>
              ...
              <td>
                <router-link :to="'/files/edit?id='+item.file_id" class="btn">
                  <b-icon icon="pencil" scale="1" variant="white"></b-icon>
                </router-link>
                <button class="btn" @click="deletes(item=item.file_id)">
...
export default {
  ...
  methods: {
    ...
    deletes(item) {
      ...
      if (isBoss == true) {
        this.$http.delete('/api/files/' + item + '/',
            {
              headers: {
